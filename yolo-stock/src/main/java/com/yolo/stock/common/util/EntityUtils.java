...
public class  EntityUtils {
    ...
    public static <T> void setCreateInfo(T entity) {
        ...
        String createBy = "";
        if (request != null) {
            createBy = StringUtils.trimToEmpty(request.getHeader("userId"));
        }
        if (StringUtils.isBlank(createBy)) {
            //todo
//            createBy = BaseContextHandler.getUserID();
            createBy = "-1";
        }

        // 默认属性
        String[] fields = {"createBy", "createTime", "enable"};
        Field field = ReflectionUtils.getAccessibleField(entity, "createTime");
        // 默认值
        Object[] value = null;
        if (field != null && field.getType().equals(Date.class)) {
            value = new Object[]{Integer.valueOf(createBy), new Date(), 1};
        }
        // 填充默认属性值
    ...
    public static <T> void setUpdatedInfo(T entity) {
        ...
        String updateBy = "";
        if (request != null) {
            updateBy = StringUtils.trimToEmpty(request.getHeader("userId"));
        }
        if (StringUtils.isBlank(updateBy)) {
//            updateBy = BaseContextHandler.getUserID();
            updateBy = "-1";;
        }

        // 默认属性
        String[] fields = {"updateBy", "updateTime"};
        Field field = ReflectionUtils.getAccessibleField(entity, "updateTime");
        Object[] value = null;
        if (field != null && field.getType().equals(Date.class)) {
            value = new Object[]{Integer.valueOf(updateBy), new Date()};
        }
        // 填充默认属性值
        setDefaultValues(entity, fields, value);
    }
